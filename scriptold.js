...
var Team = /** @class */ (function () {
    ...
    Team.prototype.getTeamScore = function () {
        for (var i = 1; i <= 10; i++) {
            this.teamscore += parseInt(document.getElementById("t".concat(this.id, "p").concat(i, "-total")).innerHTML);
        }
...
document.getElementById("hit-".concat(game1.team)).addEventListener('click', function () {
    playerChange(playerArr1, game1.team);
});
var game2 = new Game(2);
var playerArr2 = game2.gameStart();
console.log(playerArr2);
document.getElementById("hit-".concat(game2.team)).addEventListener('click', function () {
...
function playerChange(playerArr, teamnum) {
    if (playerArr[0].ballcount < 7 && !playerArr[0].isOut) {
        ...
        if (teamnum == 1) {
            document.getElementById("score-".concat(teamnum)).innerHTML = game1.getTeamScore().toString();
            document.getElementById('hit-2').removeAttribute('disabled');
            document.getElementById('hit-1').setAttribute('disabled', 'true');
        }
        else {
            document.getElementById("score-".concat(teamnum)).innerHTML = game2.getTeamScore().toString();
...
function player(playerobj, teamnum) {
    var td = document.getElementById("t".concat(teamnum, "p").concat(playerobj.id, "b").concat(playerobj.ballcount));
    ...
    if (temp == 0) {
        ...
        var td1 = document.getElementById("t".concat(teamnum, "p").concat(playerobj.id, "-total"));
