...
var TeamClass = /** @class */ (function () {
    function TeamClass(name) {
        ...
        this.teamName = name;
        for (var i = 1; i < 11; i++) {
            this.player.push(new PlayerClass("player-".concat(i)));
        }
    }
    return TeamClass;
}());
var GameClass = /** @class */ (function () {
    ...
    GameClass.prototype.getTeamScore = function () {
        ...
        for (var i = 1; i <= this.currTeam.player.length; i++) {
            teamscore += parseInt(document.getElementById("t".concat(this.currTeam.teamid, "p").concat(i, "-total")).innerHTML);
        }
...
function updateballscore() {
    if (ballnum <= 6 && playernum < 10 && !game.currTeam.player[playernum].isOut) {
        var td = document.getElementById("t".concat(currteamid, "p").concat(playernum + 1, "b").concat(ballnum));
...
function switchteam() {
    ...
    if (currteamid === 1) {
        document.getElementById("score-".concat(currteamid)).innerHTML = playernum < 9 ? pscore.toString() : game.getTeamScore().toString();
...
function printscore() {
    var tot = document.getElementById("t".concat(currteamid, "p").concat(playernum + 1, "-total"));
